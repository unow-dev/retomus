...
class SharedCtx extends Ctx {
   ...
   createHooks() {
      const useCtxStateIn = ctx => (key: string) => {
         const [state, setState] = useState(ctx.getValue(key, 'states'));
         const unsubscribe = useMemo(
            () => ctx.subscribeState(key, setState),
            [setState],
         );
         useEffect(() => {
            return unsubscribe;
      ...
      const useCtxRefIn = ctx => (key: string) => {
         const ref = useRef(ctx.getValue(key, 'refs'));
         return ref;
      };
      const useCtxFlagIn = ctx => (key: string) => {
         const [flag, setFlag] = useState(ctx.flagBus[key]);
         const unsubscribe = useMemo(
            () => ctx.subscribeFlag(key, setFlag),
            [setFlag],
         );
         useEffect(() => {
            return unsubscribe;
